...
DESTINATION_DYNAMIC = 'dynamic'
DESTINATIONS = ('qtest3_short', 'qtest3', 'qwork', 'qwork_extended', 'qfat256', 'qfat512' )
...
def old_dynamic_queue_ip07 (user_email, tool,tool_id, job, app):
    ...
    if tool_id in UPLOAD_TOOLS:
        ...
        message = launch_time[:-4] +  " job_id=" + str(job.id) +  " walltime=36:00:00 pmem=2000 -l nodes=1:ppn=1" +  " destination=" + destination.params['destination'] +  " tool_id="+ tool_id + "\n"
        print_log(message)
    ...
    if '__job_resource' in param_dict:
        if param_dict['__job_resource']['__job_resource__select'] == 'yes':
            ...
            if param_dict['__job_resource']['queue']:
                queue = param_dict['__job_resource']['queue']
                
        else:
            print "Sending tool %s job %s to %s (default destination)" % (tool_id, job.id, DESTINATION_DEFAULT)
            message = launch_time[:-4] +  " job_id=" + str(job.id) +  " walltime=72:00:00 pmem=32000 -l nodes=1:ppn=1" +  " destination=" + DESTINATION_DEFAULT +  " tool_id="+ tool_id + "\n"
            print_log(message)
