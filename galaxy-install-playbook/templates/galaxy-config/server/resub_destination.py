...
def dynamic_queue_ip07 (user_email, tool,tool_id, job, app):
    ...
    if '__job_resource' in param_dict:
        if param_dict['__job_resource']['__job_resource__select'] == 'yes':
            ...
            message = launch_time[:-4] +  " job_id=" + str(job.id) +  " walltime=22:00:00 pmem=32000 -l nodes=1:ppn=1" +  " destination=" + DESTINATION_DEFAULT +  " tool_id="+ tool_id + "\n"
            print_log(message)
...
def print_log(message):
    with open('/srv/galaxy/job_destination.log', 'ab+') as f:
        f.write(message)